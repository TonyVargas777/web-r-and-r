...
function Formulario({ onAddFilm }) {
  ...
  const handleSubmit = (event) => {
   ...
   axios
    ...
    .finally(() => {
      setFormSubmitted(true); // Update the state immediately
      setTimeout(() => {
        resetForm();
      }, 3000);
    });
        ...
        <div className="flex flex-col border font-bold place-items-center bg-azul-rodar text-white rounded-3xl mt-2  w-5/6">
          ...
          >
            ...
            <label className="block font-big text-red-700 text-red-500 w-full">
              Sinopsis:
              <textarea
                className=" mb-2 bg-red-50 border border-red-500 w-full text-red-900 placeholder-red-700 text-sm rounded focus:ring-red-500 focus:border-red-500 block w-full dark:bg-red-100 dark:border-red-400 text-center "
                value={sinopsis}
